...
The market snapshot uses the latest date on which every required daily rate
series is *jointly* available, so no tenor is ever borrowed from a different
day to claim a fresher timestamp. CPI is monthly and carries its own
observation month rather than being forced onto the daily date; the two are
always displayed separately.

**Dates are not hardcoded anywhere in this README.** The authoritative
freshness indicators are generated by the pipeline:
`public/data/data_manifest.json` (`data_as_of.rates` and `data_as_of.inflation`),
`public/data/market_snapshot.json` (`as_of_date` and
`inflation.latest_cpi_date`), and the deployed site, which prints
`rates through <date> · CPI through <month>` in the header, the data-manifest
panel and the footer.

Each scenario's real vs breakeven split is estimated per tenor from three years
of daily co-movement between TIPS and nominal yields, except where the split is
definitional (a pure real-rate or pure breakeven shock); that window rolls
forward as the sample grows. The 2022 replay is a **fixed** historical episode:
the measured nominal and real curve change between the last business days of
2021 and 2022 (2021-12-31 to 2022-12-30). It does not move when new data
arrives.
...
- price-yield inversion, and duration / convexity strictly monotonic in
  maturity across the synthetic universe.
...
# optional: print the reconciliation audit and the refresh integrity gate
python3 research/scripts/quant_audit.py
python3 research/scripts/refresh_guard.py
```

Requires Node 18+ and Python 3.9+ with the dependencies in
`research/requirements.txt` (numpy, pandas). The pipeline writes to
...
`.github/workflows/weekly-data-refresh.yml` re-runs the same pipeline every
Saturday at **11:23 UTC** (and on demand via `workflow_dispatch`), so the
published build tracks newly available public data without any change to the
methodology. The scheduled run never uses `--skip-fetch`.

```
Saturday Action → fetch FRED → run_pipeline.py → sanity_checks.py
               → quant_audit.py → refresh_guard.py → npm ci && npm run build
               → commit real changes to main → Netlify redeploys
```

Every gate is blocking. If FRED is unreachable, a series comes back short or
stale, the pipeline fails, any sanity check, reconciliation or integrity
invariant fails, or the production build fails, the run stops before staging
anything and the deployed site is left untouched. A run that finds no new
observations commits nothing — the manifest's wall-clock `generated_at` stamp
alone is not treated as a change.

`research/scripts/refresh_guard.py` is the refresh-specific gate: raw schema and
row-count regressions, future-dated or partially-quoted observations, NaNs at
the sample boundary, calibration parameters outside their documented ranges,
`public/data` drifting from `data/processed`, manifest/snapshot disagreement,
and drift in the fixed 2022 replay window. Historical revisions from FRED are
preserved rather than suppressed, and the guard reports new versus revised rows
